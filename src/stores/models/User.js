...
export default User = types
  ...
  .actions((self) => ({
    hydrate: flow(function* () {
      ...
      if (self.avatar) {
        // expo-image handles caching automatically, no need to preload
      }
    ...
    fetch_recent_tags: flow(function* () {
      ...
      console.log(
        "User:fetch_recent_tags:count",
        self.bookmark_recent_tags.length
      );
    }),
    ...
    fetch_tags_for_bookmark: flow(function* (id) {
      ...
      if (data !== API_ERROR && data.items[0] != null) {
        self.temporary_tags_for_bookmark = self.tags_to_array(
          data.items[0].tags
        );
      }
      console.log(
        "User:temporary_tags_for_bookmark:array",
        self.temporary_tags_for_bookmark
      );
      self.is_fetching_tags_for_bookmark = false;
    }),
    ...
    set_selected_temp_tag: flow(function* (tag) {
      ...
      const existing_tag = self.temporary_tags_for_bookmark.find(
        (t) => t === tag
      );
    ...
    delete_selected_temp_tag: flow(function* (tag) {
      ...
      const existing_tag_index = self.temporary_tags_for_bookmark.findIndex(
        (t) => t === tag
      );
    ...
    set_selected_temp_tag_from_input: flow(function* () {
      console.log(
        "User:set_selected_temp_tag_from_input",
        self.bookmark_tag_filter_query
      );
      const existing_tag = self.temporary_tags_for_bookmark.find(
        (t) => t === self.bookmark_tag_filter_query
      );
      if (existing_tag == null) {
    ...
    update_tags_for_bookmark: flow(function* () {
      ...
      self.set_bookmark_tag_filter_query(null);
      const data = yield MicroBlogApi.save_tags_for_bookmark_by_id(
        self.temporary_bookmark_id,
        self.temporary_tags_for_bookmark.toString()
      );
      if (data !== API_ERROR) {
    ...
    prep_posting: flow(function* () {
      ...
      if (
        ...
        (self.is_premium != null && !self.is_premium) ||
        (self.is_using_ai != null && !self.is_using_ai)
      ) {
  ...
  .views((self) => ({
    ...
    filtered_tags() {
      return self.bookmark_tag_filter_query != null &&
        self.bookmark_tag_filter_query != "" &&
        self.bookmark_tags.length > 0
        ? self.bookmark_tags.filter((tag) =>
            tag.includes(self.bookmark_tag_filter_query)
          )
        : self.bookmark_tags;
    },
    ...
    is_registered_for_push() {
      return (
        Push.is_registered_device_for_user(self.username) && self.push_enabled
      );
    },
  }));
