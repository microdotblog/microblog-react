...
export default class PostCell extends React.Component {
  ...
  _trigger_delete = () => {
    Auth.selected_user.posting.selected_service?.trigger_post_delete(
      this.props.post
    );
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <TouchableOpacity
                onPress={() => App.handle_url_from_webview(post.url)}
              >
                <Text style={{ color: "gray", fontSize: 12 }}>
                  {post.nice_local_published_date()}
                  {post.is_draft() ? <Text> — draft</Text> : null}
                </Text>
              </TouchableOpacity>
              {Platform.OS === "android" && (
                <TouchableOpacity
                  onPress={this._trigger_delete}
                  style={{ flexDirection: "row", alignItems: "center" }}
                >
                  <Text style={{ color: "rgb(239,68,68)", fontSize: 15 }}>
                    Delete...
                  </Text>
                </TouchableOpacity>
              )}
