	...
	render() {
		...
		return (
			<Swipeable
				...
				renderRightActions={(progress) => this._right_actions(progress, collection)}
			>
			<View
				key={collection.id}
				style={{
			...
			>
				<Text style={{ color: App.theme_text_color() }}>{collection.name}</Text>
				<Text style={{ color: App.theme_text_color() }}>{collection.uploads_count}</Text>
			</View>
			</Swipeable>
		)
	}
