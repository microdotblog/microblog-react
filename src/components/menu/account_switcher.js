...
export default class AccountSwitcher extends React.Component {
  _render_current_user = () => {
    return (
      <TouchableOpacity
        ...
        accessibilityLabel={`Profile for ${Auth.selected_user.username}`}
        style={{
          flexDirection: "row",
          ...
          return (
            <TouchableOpacity
              ...
              style={{
                ...
                marginLeft: 16,
                paddingBottom:
                  index === Auth.all_users_except_current().length - 1 ? 0 : 18,
              }}
            >
              <View
                style={{
          ...
          {Platform.OS === "ios" ? (
            ...
            <Image
              style={{
                ...
                marginRight: 18,
                marginLeft: 6,
                tintColor: App.theme_button_text_color(),
              }}
              source={AddAccountImage}
            />
          )}

          <Text style={{ color: App.theme_button_text_color(), marginLeft: 6 }}>
            Add Account...
          </Text>
        </View>
      </TouchableOpacity>
  ...
  _render_account_logout_button = () => {
    const sign_out_wording =
      Auth.users?.length > 1
        ? `Sign Out of @${Auth.selected_user.username}`
        : "Sign Out";
    return (
      <TouchableOpacity
      ...
      >
        ...
        >
          <Text style={{ color: App.theme_accent_color() }}>
            {sign_out_wording}
          </Text>
        </View>
      </TouchableOpacity>
